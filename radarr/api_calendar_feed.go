...
func (r ApiGetFeedV3CalendarRadarrIcsRequest) ReleaseTypes(releaseTypes []CalendarReleaseType) ApiGetFeedV3CalendarRadarrIcsRequest {
	r.releaseTypes = &releaseTypes
	return r
}
...
func (a *CalendarFeedAPIService) GetFeedV3CalendarRadarrIcsExecute(r ApiGetFeedV3CalendarRadarrIcsRequest) (*http.Response, error) {
	...
	if r.releaseTypes != nil {
		t := *r.releaseTypes
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "releaseTypes", s.Index(i).Interface(), "form", "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "releaseTypes", t, "form", "multi")
		}
	}
